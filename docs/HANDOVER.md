...
| Profiling | `Instrumentor` | `GS_PROFILE_SCOPE`, live panel, Chrome trace |
...
./gs.py run release -- --play run.rec --hide-ui
```
...
screenshots and `imgui.ini` land beside it. `./gs.py run` does that; running
...
**Sanitizers are a command now, not an afternoon.** `./gs.py sanitize` builds
with ASan and UBSan into `bin/<Config>-...-sanitize/` and runs every demo under
it in lockstep, reporting per demo; `./gs.py sanitize release` does it at -O2,
...
- A per-call `GS_PROFILE_SCOPE` inside `Raycast` cost more than the raycast,
...
<!-- trap-index: ./gs.py traps regenerates this -->

### Index

One line per trap, generated by `./gs.py traps`. Scanning this costs about
4k tokens against 31k for the section itself -- `grep -n` the phrase to land on
the trap that explains it. **Do not edit by hand.**

- A `mat4` vertex attribute occupies four locations
- An instance buffer becomes part of a mesh's vertex array
- Instancing a thing that moves every frame saves draw calls and not CPU; instancing a thing that stands still saves both
- The way to check a rendering refactor is to render it both ways and compare the pixels
- A frame time that does not move may be vsync
- The generated project files are global state, and a sanitizer sweep takes minutes
- A Python traceback quotes the source file as it is *now*
- A conversion loop written as `while (owed > 0)` rounds up, and rounding up in a salvage mints material
- Test a geometric predicate somewhere empty
- A counter of what the player did must not be decremented by undoing something they did not do
- A mean is the wrong statistic for a rule with a radius
- A random walk's noise has to be scaled by the square root of the timestep
- Step a flock from a copy of where it was, not in place
- A file format needs a version before it needs anything else
- Keep a built-in kit separate from the list the user edits
- A function's parameter type must be complete where the function is declared
- Dark metal indoors comes out black
- Snap the lower corner to a lattice, never the centre
- "Is it in that plane" is not "is its face on that plane"
...
- When a unit changes with the axis, say so in the field's name or its comment
- `GroundBelow`'s and `GroundHeightBelow`'s last argument is the *initial best*, not a floor to search from, and it defaults to `0.0`
- A body drawn about a different point than it is simulated about needs that offset stored somewhere
- A spin and a linear velocity picked separately describe two different motions
- A post-step projection applied before the step is a projection the step undoes
- A kinematic body is a body the solver has been told not to move
...
- A per-object lighting term missing from a shader can hide for months behind what the shader happens to draw
- Two boxes that span the same range interpenetrate, and coplanar faces z-fight
- A terrain modifier belongs in `Height`, not in `Density`
- A packed bit field one bit too narrow does not fail, it forgets
- Clamping a quantity to make it fit is a silent discount
- Never write to a registered replay parameter from the simulation
- A pool of bodies is not cleared by `PhysicsWorld3D::Clear`, it is *orphaned* by it
- An index into `m_Loose` held anywhere else has to survive a removal
- A registered replay parameter is a raw pointer kept for the life of the demo
- `GroundHeightBelow`'s last argument is the initial best, not a floor to search from
- A constant reading across a changing input is a bug in the measurement more often than in the code
- A pool of bodies, not a growing list
- A self-test that counts a converted thing twice
- Indexing a lazily filled pool from a test
- Never seed scattered things on a triangle's index
- Placement rules that read the mesh describe something the player can edit
- Buoyancy applied at the centre of mass can never tip anything
- A 38 mm plank is thinner than a body moves in one step
- Viscous damping gives a terminal speed, not a stop
- A tree's foliage is at chest height, and anything keyed to a height on the tree will find it there
- A basis matrix built from `cross` twice is left-handed as often as not, and a left-handed transform draws the mesh inside out
- A portal's second camera needs its near plane in the plane of the doorway, or it draws what is between it and the door
- `0.5 + 0.5 * n.y` is not an ambient term, it is half of one
- A sphere on a slope never stops, and no amount of friction changes that
- `Texture2D::SetSmooth(true)` on a framebuffer attachment makes it sample black
- A constant that reaches a shader as a literal will drift, and the symptom will not look like the cause
- Distance for a level-of-detail term must be from the eye, and it is easy to write one that is not
- `--land Mars` separates the player from the lander, and it is not the lander
- A 1/f fractal has no energy at the scale you can see, and that is not a bug in the fractal
- Sampling error scales with `A / L^2`, not with `L`
- A noise field is only a field at the scale you sample it
- A distance to the landing site is not a distance on the ground
- An ambient floor put there to prevent black produces black
- Rebuilding a derived thing while its input streams in is quadratic
- Attribute before optimising, every time
...
- A least-squares fit over a band assumes the thing being fitted is flat across it
- Detection and attribution are different questions
- A `mat4` vertex attribute is four locations, not one
- Attribute locations are handed out in the order vertex buffers are added, so an instance buffer cannot be grown
- What differs between instances goes in the buffer; what is *shared* must still be a uniform
- A GPU falling to idle during a stall means the CPU is the bottleneck
- A work budget has to count the work, not the successes
- A budget whose atom is expensive needs a fractional allowance
- Anything the physics reads is part of the simulation, however much it looks like scenery
- A near plane derived from the height above the *mean radius* clips the ground you are standing on
- `shape * Amplitude - bias` is skewed, so ±A/2 does not bound it
- A bit mask does not overflow, it wraps
- `fwidth` after a branch is undefined, and it costs reproducibility rather than correctness
- `fract(sin(dot(v, k)) * 43758.5)` breaks down at large arguments
- A point source narrower than a pixel is not a small dot, it is an aliased one
- An additive blend cannot hide anything, so additive air is not air
- Single scattering makes optically thick media dark, not bright
- A fixed step count over a path length that jumps is two different quadratures
- `normalize` of a point on a view ray is not safe when the ray points at the centre
- A Fibonacci spiral is ordered by z, so the first half of it is one hemisphere
- A rate measured per *outcome* is not a rate
- Every landing site is dry, so "is it dry" cannot catch a dropped rotation
- An unallocated voxel chunk reads as air, not as rock
- A chunk meshed before its high neighbours exist is not a crack, it is a wall — and repairing it later is not the same as never drawing it
- `OnDemoActivated` runs after the frame's fixed steps, not before them
- Reading the mouse without a gate breaks step determinism
...
- `/tmp` is a tmpfs here, so a log file is resident memory
- A changelog entry can describe work that was never committed
- A test suite that passes on the first run has not been tested
- glTF: the stride is on the bufferView, not implied by the element
- glTF component orders that are wrong-but-valid
- The inverse transpose can be invisible and still be right
- "Different seed, different output" only proves *some* seed dependence
- A predictor that assumes one body per patch of ground
- A subdivided face needs subdivided *edges*
- Geometric coincidence is not topological agreement
- Area and volume cannot see a topology error
- The LOD seam is a step, not a crack
- `MarchingCubes` and `MarchingTetrahedra` do not agree on a shared face
- `flat` is a GLSL keyword
- Sphere tracing and a sparse field do not mix
- A test that moves the world has to put it back where it found it
- A first-person body has to agree with the camera's own height
- A kinematic body overlapping a dynamic one is catastrophic, not marginal
- Zero drift can mean the test never reproduced the bug
- A guard in the caller is not a rule
- A test that drives the collaborators itself never checks the wiring
- A mutation that survives may have mutated the wrong line
- A sphere collider rolls forever
- Render state is global and outlives the demo that set it
- Crossing a boundary needs to look like crossing it
- A signed field scales both sides
- A capture that stops at step 300 cannot see a trend
- A comment doing a constant's job
- A light albedo has no cel bands
- "It never moved" is a test-setup failure, not a physics failure
- A cache keyed by a version number is a trap; key it by behaviour
- Round-trip tests miss whatever the *setup* cannot reach
- Streaming order is invisible until the radius grows
- A green suite can be measuring a feature that does nothing
- A test that cannot see the line it is testing
- A constant offset is the measurement, not the code
- Adding a field to a struct silently rewrites every positional initialiser
- Which way does yaw go?
- Depth, in 2D
- Separate the peak from the settled value before blaming the solver
...
- A manifold must describe what is touching, not what the shape spans
- A resting test dropped level proves nothing
- Contact order is part of the answer
- Cube3D's capture depends on the mouse cursor position
- `--demo` takes the short name from `DemoRegistry.h`, not the class name
- ImGui config flags are read once, in `OnAttach`
- `OnAttach` runs for every pushed layer
- Member initialisation order follows declaration order
- Every layer sees the same event
- ImGui takes keys before you do
- Framebuffers with two draw buffers
- `glClear` will not clear an integer attachment
- Picking stores the slot index, not the `EntityId`
- `AudioEngine::StopAll` only *requests* a stop
- A constant dB offset across every measurement is the measurement's fault
- float32 bottoms out around −100 dB in the audio path
- The audio thread must never allocate, lock, or block
- A cube's tilt cannot be measured from its local up axis
- Do not rotate bodies in 3D position correction
- A clipped face has to be wound about its own outward normal
- Sleeping can hide a broken stack
- Counting open edges means deciding which vertices are one vertex, and snapping to a grid does it wrong in both directions
- A ratio measured across two changes belongs to both of them
- A one-sided oracle asserted both ways will contradict a correct answer
- Parallel axes give a zero-length cross product
- In 3D, angular momentum is conserved and angular velocity is not
- The tennis racket theorem is the test
- Anything that moves belongs in `OnFixedUpdate`, not `OnUpdate`
- `Step` must integrate positions *after* the contact solve
- `DrawRotatedQuad` takes degrees
- Anything deriving bounds from `HalfExtents` has to account for rotation
- Cutting too much when editing a demo header
- A ground probe's origin has to travel with the thing probing
- Friction does not constrain the character
- A test course has to outlast the thing being tested
- `Terrain::NormalAt` is a central difference, and `HeightAt` answers 0 off the map
- Anything drawn that follows the mouse breaks capture reproducibility
- `glPointSize` does nothing while `GL_PROGRAM_POINT_SIZE` is enabled
- A walking capsule is dynamic with its rotation thrown away
- A rigid body's position has to be its centre of mass
- A compound's inertia tensor is not diagonal
- A moment belongs only to a section that is the sole path to ground
- A Jacobi sweep can return a reflection, not a rotation
- Region-bounding the stress analysis does not work
- `VoxelField3D::Far` is a sentinel, not a distance
- A structural strength has to clear the terrain's own weight
- Fade a subtracted field towards +big, never towards zero
- Shapes meant to join have to overlap, at both ends
- Voxel counting measures the shape shrunk by half a voxel on every face
- `yaw = 0` points along +x
- `y - h(x, z)` is not a distance field
- The same error at two different conditions is arithmetic, not physics
- Check that a tangent is tangent
- A ball on any interesting surface rolls away
- An identical failure count across two resolutions is the test, not the code -- until you measure which
- A voxel field must be Lipschitz
- Mesh chunks with a one-cell overlap
- A replay only reproduces the sliders a demo registered
- Registration order is load-bearing at both ends
- `--capture` and `--play` open no window
- Anything drawn from the cursor breaks capture reproducibility, and Cube3D had it too
- A room assembled from slabs leaks at every joint that does not overlap
- A broadphase cost that does not scale with the body count is not being paid for the bodies
- `BroadphaseMinBodies` cannot be tuned from one scene
- "Not active" is not "not in use", and the audio engine believed otherwise for a long time
- Silence from a race detector is only evidence if the window was open
- A sanitizer report names a stack, and only its innermost frame did the thing
- Classify sanitizer reports, do not suppress them
- A protocol bug in the mixer is as likely to be silence as a race
- A star's apparent colour is not the light it emits
- `LinearDamping` defaults to 0.01, and it destroys orbits silently
- World gravity is one vector, which a planet is not
- A yaw/pitch camera cannot stand on a sphere
- A sphere of the wrong radius looks exactly like a sphere
- Filling a voxel chunk stales its low neighbours' meshes
- On a Wayland session, X11 sees almost nothing
- KWin reports logical pixels; XWayland lives in physical ones
- `%*[^,]` does not match an empty field
- A heightfield has two boxes, and using the wrong one drops pairs
- An infinity in a bounds box is safer than a large sentinel
- Comparing a raycast against "the first sample under the surface" only works if the ray starts above the surface
- `GroundHeightBelow`'s default floor is 0
- No texture in this engine had a mip chain until 2026-08-26
- An object with correct geometry, a correct transform, and a working shader can still render as nothing, for a reason that isn't in the draw call
- A pixel count only means something if the pixels came from the shader you are measuring
- Check a debug encoding's range before believing what it says
- A cache keyed by "what the generator does" has to sample the code path the cache actually stores, not something next to it
- `memcpy` of four bytes into an eight-byte variable hashes four bytes of the stack, and neither ASan nor UBSan will tell you
- A constant tuned at one scale is wrong at every other scale, and an `exp(-x)` hides it by saturating quietly
- This bug was invisible because two other bugs were compensating for it
- `atan`-built sphere UVs collapse the mip chain along the seam
- A circle standing back for a square leaves a ring
- The Solar demo's rocks roll downhill for ever, and it is not the wind
- One factor cannot do both kinds of heat redistribution
- Below a voxel field, an SDF collider reads solid for ever
- The editor viewport is a `glViewport` call, not a framebuffer, and that is deliberate
- ImGui runs in layer order
- MSAA is on now: four samples
- ~~There is no MSAA in this engine~~ (fixed)
- Sub-pixel geometry aliases in its *shading*, not its silhouette
- Flat normals are why anything built as a jittered low-poly sphere looks blocky
- Put input that edits or teleports on the fixed step, not on events
- The planet's buried trees are not a placement-frame problem
- A CPU-side count of what was submitted says nothing about what was drawn
- A `.replace` across a file that holds five shaders will hit more than one of them
- `flat` is a GLSL interpolation qualifier
- Both sides of a stochastic LOD test must be constant across the primitive
- When something in the near field looks like a "geometry artifact", check the scale of the small props before suspecting the terrain
- "World space" in the Solar demo is camera-relative, and anything that wants a *fixed* position must say so
- The default landing site is a hard-coded direction, and any change to `Relief` invalidates it
...
- `Colour` in the body table is not an albedo
- `AtmosphereFraction * AtmosphereDensity` are render depths, not weights of gas
- `./gs.py sanitize` reports 16 of 16 demos FAIL and it means nothing
...
- **The generated project files are global state, and a sanitizer sweep takes
  ...
  asked for, so a plain `./gs.py build` started while a sweep is running
...
- **A Python traceback quotes the source file as it is *now*.** Having edited
  `gs.py` while a sweep was running, every line the traceback quoted came
...
- **`/tmp` is a tmpfs here, so a log file is resident memory.** Probing the
  above with one `GS_INFO` per loop iteration wrote a 1.6 GB log into the
...
- **Adding a field to a struct silently rewrites every positional initialiser.**
  ...
  boundary has the identical signature; `./gs.py clean` disproved that in one
...
- **On a Wayland session, X11 sees almost nothing.** `_NET_CLIENT_LIST` held two
  ...
  D-Bus, bridged by `tools/gs-windows.py`, because KWin scripts can neither
...
- **Put input that edits or teleports on the fixed step, not on events.**
  Events are not in the replay stream and `GS::Input` is, so a digging session
...
match (32 slots on this machine, was a hardcoded 16); `gs.py` fetches a
...
`Terrain::HeightAt` answers from `GS::Heightfield3D` now so there is one
...
- **Debug is about seven times slower than Release on the streaming path**, and
  `./gs.py run` builds Debug. 40 FPS against 60 for the same landed scene. Do
