...
namespace Egss {
	...
	SubTexture2D* SubTexture2D::CreateFromCoords(const std::shared_ptr<Texture2D>& texture,
		const glm::vec2& coords, const glm::vec2& cellSize, const glm::vec2& spriteSize)
	{
		// Pixel rect converted to normalised coordinates. Texture space runs
		// bottom-up, matching the flip applied when images are loaded.
		glm::vec2 min = {
