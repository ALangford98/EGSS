...
INDEX_BEGIN = "<!-- trap-index: ./egss.py traps regenerates this -->"
...
def build_trap_index(lines):
    ...
    try:
        ...
        sys.exit("docs/HANDOVER.md no longer has the headings the index is "
                 "built from; fix TRAPS_HEADING/TRAPS_END in egss.py.")
    ...
    block = [
        ...
        "### Index",
        "",
        "One line per trap, generated by `./egss.py traps`. Scanning this "
        "costs about",
...
def trap_index(check=False):
    ...
    if check:
        print("trap index is stale -- run ./egss.py traps")
        return 1
