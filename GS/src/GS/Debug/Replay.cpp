...
namespace GS {
	...
	namespace {
		...
		// changing it would make an older build report "not an GS recording",
		...
		void ResolveParamMapping()
		{
			...
			for (const std::string& name : s_ParamNames)
			{
				...
				{
					missing++;
					GS_CORE_WARN("Replay: this build has no parameter '{0}'", name);
				}
			...
			if (missing > 0)
			{
				...
				GS_CORE_WARN("Replay: {0} of {1} recorded parameters are missing;"
	...
	bool Replay::StartRecording(const std::string& path, int demoIndex, float fixedTimestep)
	{
		...
		{
			GS_CORE_ERROR("Replay: cannot record while playing back; ignoring --record");
			return false;
		}
		...
		if (!s_Out)
		{
			GS_CORE_ERROR("Replay: could not open '{0}' for recording", path);
			return false;
		}
		...
		for (size_t i = 0; i < ReplayParams::Count(); i++)
		{
			...
			{
				GS_CORE_WARN("Replay: parameter name '{0}' is longer than {1} and"
		...
		GS_CORE_INFO("Replay: recording to '{0}' (demo {1}, step {2:.5f}s, {3} parameters)",
	...
	bool Replay::StartPlayback(const std::string& path)
	{
		...
		if (!in)
		{
			GS_CORE_ERROR("Replay: could not open '{0}' for playback", path);
			return false;
		}
		...
		{
			GS_CORE_ERROR("Replay: '{0}' is not an GS recording", path);
			return false;
		}

		if (header.Version != s_Version)
		{
			GS_CORE_ERROR("Replay: '{0}' is version {1}, this build reads {2}",
		...
		for (unsigned int i = 0; i < header.ParamCount; i++)
		{
			...
			{
				GS_CORE_ERROR("Replay: '{0}' ends inside its parameter table", path);
				return false;
			}
		...
		while (in.read(reinterpret_cast<char*>(&tag), sizeof(tag)))
		{
			...
			else
			{
				...
				GS_CORE_ERROR("Replay: '{0}' has an unknown chunk tag {1}; stopping"
		...
		GS_CORE_INFO("Replay: playing '{0}' -- demo {1}, {2} steps, {3} input records,"
	...
	void Replay::Stop()
	{
		if (s_Mode == Mode::Recording && s_Out.is_open())
		{
			...
			GS_CORE_INFO("Replay: recorded {0} steps to '{1}'", s_RecordedSteps, s_OutPath);
		}
