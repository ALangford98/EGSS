...
namespace GS {
	...
	bool ScreenCapture::SaveFrame(const std::string& path, unsigned int width, unsigned int height)
	{
		if (width == 0 || height == 0)
		{
			GS_CORE_ERROR("ScreenCapture: refusing to capture a {0}x{1} frame", width, height);
			return false;
		}
		...
		if (written == 0)
		{
			GS_CORE_ERROR("ScreenCapture: could not write '{0}'", path);
			return false;
		}

		GS_CORE_INFO("ScreenCapture: wrote '{0}' ({1}x{2})", path, width, height);
		return true;
	}
