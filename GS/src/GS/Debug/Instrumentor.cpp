...
namespace GS {
	...
	void Instrumentor::BeginSession(const std::string& name, const std::string& filepath)
	{
		...
		{
			GS_CORE_WARN("Instrumentor: session already running, ignoring BeginSession('{0}')", name);
			return;
		}
		...
		{
			GS_CORE_ERROR("Instrumentor: could not open '{0}' for writing", filepath);
			return;
		}
		...
		GS_CORE_INFO("Instrumentor: capturing to '{0}'", filepath);
	}
	...
	void Instrumentor::EndSession()
	{
		...
		GS_CORE_INFO("Instrumentor: wrote {0} events to '{1}'", state.WrittenCount, state.SessionPath);
	}
