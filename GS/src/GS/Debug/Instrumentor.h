...
namespace GS {
	...
	class GS_API InstrumentationTimer
	{
	public:
...
// Compiled out entirely in Dist, so shipping builds carry no timing code.
	...
	#else
		#define GS_FUNC_SIG "unknown"
	#endif

	// Two levels of indirection so __LINE__ expands before pasting, giving
	// each timer in a scope a distinct variable name.
	#define GS_PROFILE_JOIN2(a, b) a##b
	#define GS_PROFILE_JOIN(a, b) GS_PROFILE_JOIN2(a, b)

	#define GS_PROFILE_SCOPE(name) ::GS::InstrumentationTimer GS_PROFILE_JOIN(gsTimer, __LINE__)(name)
	...
	#define GS_PROFILE_BEGIN_SESSION(name, path) ::GS::Instrumentor::BeginSession(name, path)
	#define GS_PROFILE_END_SESSION() ::GS::Instrumentor::EndSession()
#else
	#define GS_PROFILE_SCOPE(name)
	#define GS_PROFILE_FUNCTION()
	#define GS_PROFILE_BEGIN_SESSION(name, path)
