...
namespace GS {
	...
	Application::Application()
	{
		...
		if (!props.Visible)
			GS_CORE_INFO("No window: unattended run (--show-window to watch it)");
	...
	void Application::ParseCommandLine()
	{
		...
		if (m_Lockstep)
		{
			...
			m_Window->SetVSync(false);
			GS_CORE_INFO("Lockstep: one fixed step per frame, VSync off");
		}
		...
		if (!m_CapturePath.empty())
		{
			...
			if (m_CaptureStep != 0)
				GS_CORE_INFO("Capture scheduled: '{0}' at step {1}", m_CapturePath, m_CaptureStep);
			else
				GS_CORE_INFO("Capture scheduled: '{0}' at frame {1}", m_CapturePath, m_CaptureFrame);
		}
	}
	...
	void Application::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<WindowResizeEvent>(GS_BIND_EVENT_FN(Application::OnWindowResize));
	...
	void Application::Run()
	{
		while (m_Running)
		{
			...
			if (!m_Minimized)
			{
				...
				{
					GS_PROFILE_SCOPE("Layer::OnFixedUpdate");
				...
				{
					GS_PROFILE_SCOPE("Layer::OnUpdate");
			...
			{
				GS_PROFILE_SCOPE("ScreenCapture");

				GS_CORE_INFO("Capturing at frame {0}, step {1}", m_FrameCount, m_StepCount);
...

			{
				...
				GS_PROFILE_SCOPE("Window::OnUpdate (swap + poll)");
				m_Window->OnUpdate();
			}
			...
			{
				GS_CORE_INFO("Exiting after {0} frames as requested", m_FrameCount);
				m_Running = false;
			}
			...
			{
				GS_CORE_INFO("Replay finished at step {0}", m_StepCount);
				...
				if (!m_CapturePath.empty() && !m_Captured)
				{
					GS_CORE_ERROR("Capture at step {0} never happened: the replay is only {1} steps",
		...
		GS_CORE_INFO("Application run loop exiting");
	}
}
