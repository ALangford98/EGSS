...
$CASES$				}
		...
		auto substitute = [](std::string& src, const std::string& token, const std::string& value)
		{
			...
			GS_CORE_ASSERT(at != std::string::npos, "Renderer2D shader template lost a token");
		...
		GS_CORE_INFO("Renderer2D initialized ({0} quads/batch, {1} texture slots)",
	...
	{
		GS_PROFILE_SCOPE("Renderer2D::Flush");
