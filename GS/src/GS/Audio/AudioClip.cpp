...
namespace GS {
	...
	std::shared_ptr<AudioClip> AudioClip::Create(const std::string& path)
	{
		...
		{
			GS_CORE_ERROR("AudioClip: could not decode '{0}'", path);
			return nullptr;
		}
		...
		{
			GS_CORE_ERROR("AudioClip: '{0}' has no frames", path);
		...
		GS_CORE_INFO("AudioClip '{0}': {1} frames ({2:.2f}s)", path, read, clip->GetDurationSeconds());
		return clip;
	}
