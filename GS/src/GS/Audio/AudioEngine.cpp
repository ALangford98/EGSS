...
namespace GS {
	...
	void AudioEngine::Init()
	{
		...
		{
			GS_CORE_WARN("AudioEngine: no playback device; audio will be silent");
			return;
		}

		if (ma_device_start(&state.Device) != MA_SUCCESS)
		{
			GS_CORE_WARN("AudioEngine: device found but would not start; audio will be silent");
		...
		GS_CORE_INFO("Audio {0} | {1} Hz, {2} ch, {3} voices",
