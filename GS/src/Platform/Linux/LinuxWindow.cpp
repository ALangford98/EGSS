...
namespace GS {
	...
	void LinuxWindow::Init(const WindowProps& props)
	{
		...
		if (props.Wallpaper)
		{
			...
			if (!monitors.empty())
			{
				...
				GS_CORE_INFO("Wallpaper: {0} monitors spanning {1}x{2} at ({3}, {4})",
		...
		GS_CORE_ASSERT(gladStatus, "Could not initialize Glad");
		GS_CORE_INFO("OpenGL {0} | {1}", (const char*)glGetString(GL_VERSION), (const char*)glGetString(GL_RENDERER));
	...
	void LinuxWindow::MakeDesktopWindow(bool show)
	{
		...
		if (!display || !handle)
		{
			GS_CORE_WARN("--wallpaper: no X11 window to mark (not an X11 session?)");
			return;
		}
		...
		GS_CORE_INFO("Wallpaper mode: window 0x{0:x} marked as desktop", (unsigned long)handle);
