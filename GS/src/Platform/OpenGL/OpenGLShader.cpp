...
namespace GS {
	...
	static unsigned int ShaderTypeFromString(const std::string& type)
	{
		...
		GS_CORE_ERROR("Unknown shader type '{0}'", type);
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (!in)
		{
			GS_CORE_ERROR("Could not open shader file '{0}'", filepath);
			return result;
		}
	...
	std::unordered_map<unsigned int, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			{
				GS_CORE_ERROR("Shader syntax error: no newline after #type");
				break;
			}
	...
	void OpenGLShader::Compile(const std::unordered_map<unsigned int, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (!compiled)
			{
				...
				GS_CORE_ERROR("Shader '{0}' compilation failed: {1}", m_Name, log.data());
		...
		if (!linked)
		{
			...
			GS_CORE_ERROR("Shader '{0}' link failed: {1}", m_Name, log.data());
	...
	int OpenGLShader::GetUniformLocation(const std::string& name)
	{
		...
		if (location == -1)
			GS_CORE_WARN("Uniform '{0}' not found in shader '{1}'", name, m_Name);
