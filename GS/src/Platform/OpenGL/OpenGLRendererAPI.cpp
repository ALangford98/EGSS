...
namespace GS {
		...
		GLsizei length, const GLchar* message, const void* userParam)
	{
		switch (severity)
		{
			case GL_DEBUG_SEVERITY_HIGH:         GS_CORE_CRITICAL("GL: {0}", message); return;
			case GL_DEBUG_SEVERITY_MEDIUM:       GS_CORE_ERROR("GL: {0}", message); return;
			case GL_DEBUG_SEVERITY_LOW:          GS_CORE_WARN("GL: {0}", message); return;
	...
	void OpenGLRendererAPI::Init()
	{
		...
		if (samples > 1)
			GS_CORE_INFO("GL: multisampling on, {0} samples a pixel", samples);
		else
			GS_CORE_WARN("GL: no multisampling -- the driver gave {0} samples",
				samples);
		...
		GS_CORE_INFO("GL: {0} fragment texture units reported, using {1}",
