...
namespace GS {
		...
		: m_Path(path)
	{
		...
		if (!data)
		{
			GS_CORE_ERROR("Could not load image '{0}': {1}", path, stbi_failure_reason());
			return;
		}
	...
	OpenGLTexture2D::OpenGLTexture2D(const unsigned char* memory, unsigned int size, const std::string& name)
	{
		...
		if (!data)
		{
			GS_CORE_ERROR("Could not decode image '{0}': {1}", name, stbi_failure_reason());
			return;
		}
		...
		int channels, const std::string& name)
	{
		...
		{
			GS_CORE_ERROR("Unsupported channel count {0} in '{1}'", channels, name);
	...
	void OpenGLTexture2D::SetData(void* data, unsigned int size)
	{
		...
		GS_CORE_ASSERT(size == m_Width * m_Height * bytesPerPixel, "SetData must cover the entire texture");
